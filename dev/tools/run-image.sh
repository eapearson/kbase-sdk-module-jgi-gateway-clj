...
module="kbase-sdk-module-jgi-search-clj"
root=$(git rev-parse --show-toplevel)
config_mount=${root}/dev/docker/fake-work

docker run \
  -p 81:81 --dns=8.8.8.8 \
  --mount type=bind,src=${config_mount},dst=/kb/module/work \
  -e KB_SERVICE_NAME="jgi_gateway_clj" \
  -e KB_DEPLOYMENT_CONFIG="/kb/module/deploy.cfg" \
  -e PORT=81 \
  kbase/${module}:dev
